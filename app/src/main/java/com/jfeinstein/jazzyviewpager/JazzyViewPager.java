...
public class JazzyViewPager extends ViewPager {
    ...
    protected void animateTablet(View left, View right, float positionOffset) {
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                mTrans = getOffsetXForRotation(mRot,
    ...
    private void animateRotate(View left, View right, float positionOffset, boolean up) {
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                mTrans = (up ? -1 : 1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math.cos(
                        mRot * Math.PI / 180.0f));
                left.setPivotX(left.getMeasuredWidth() * 0.5f);
    ...
    private void animateFlipHorizontal(View left,
            View right,
    ...
    private void animateFlipVertical(View left,
            View right,
            float positionOffset,
            int positionOffsetPixels) {
        if (mState != State.IDLE) {
    ...
    protected void animateStack(View left,
            View right,
            float positionOffset,
            int positionOffsetPixels) {
        if (mState != State.IDLE) {
        ...
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                if (positionOffset < -1) {
                    ...
                    float scaleFactor = Math.max(ZOOM_OUT_IN_MIN_SCALE,
                            1 - Math.abs(positionOffset));
                    float vertMargin = pageHeight * (1 - scaleFactor) / 2;
                    float horzMargin = pageWidth * (1 - scaleFactor) / 2;

                    if (positionOffset < 0) {
                        left.setTranslationX(horzMargin - vertMargin / 2);
                    } else {
                        left.setTranslationX(-horzMargin + vertMargin / 2);
                    }
                    ...
                    left.setAlpha(ZOOM_OUT_IN_MIN_ALPHA + (scaleFactor - ZOOM_OUT_IN_MIN_SCALE) / (1 - ZOOM_OUT_IN_MIN_SCALE) * (1 - ZOOM_OUT_IN_MIN_ALPHA));
                } else {
                    left.setAlpha(0f);
                }
            }
            ...
            if (right != null) {
                ...
                if (positionOffset < -1) {
                    ...
                    float scaleFactor = Math.max(ZOOM_OUT_IN_MIN_SCALE,
                            1 - Math.abs(positionOffset - 1));
                    float vertMargin = pageHeight * (1 - scaleFactor) / 2;
                    float horzMargin = pageWidth * (1 - scaleFactor) / 2;

                    if (positionOffset < 0) {
                        right.setTranslationX(horzMargin - vertMargin / 2);
                    } else {
                        right.setTranslationX(-horzMargin + vertMargin / 2);
                    }
                    ...
                    right.setAlpha(ZOOM_OUT_IN_MIN_ALPHA + (scaleFactor - ZOOM_OUT_IN_MIN_SCALE) / (1 - ZOOM_OUT_IN_MIN_SCALE) * (1 - ZOOM_OUT_IN_MIN_ALPHA));
                } else {
                    right.setAlpha(0f);
                }
            }
